...
// Build the default watchFree structure for a list of machines.
function makeWatchFree(machines) {
  const watchFree = {};
  const hostels = Array.from(new Set(machines.map((m) => m.hostel)));
  hostels.forEach((hostel) => {
    watchFree[hostel] = {};
    const floors = Array.from(
      new Set(machines.filter((m) => m.hostel === hostel).map((m) => m.floor))
    );
    floors.forEach((floor) => {
      watchFree[hostel][floor] = false;
    });
  });
  return watchFree;
}

// Construct a fresh state snapshot.
function buildDefaultState() {
  ...
  return {
    ...
    notices: [],
    reports: [],
    watchFree: makeWatchFree(machines),
    user: null,
    rooms,
    bookings: [],
  };
...
function initState(force = false) {
  const validStatuses = ['FREE', 'RUNNING', 'AWAITING', 'MAINT'];
  const hardReset = () => {
    localStorage.removeItem('hostelsync_state');
    state = buildDefaultState();
    initCommonRoomDb(true);
    syncUserFromCookie();
    saveState();
  };

  try {
    state = force ? null : loadState();
    let needsReset = force;
    if (!state) needsReset = true;
    if (state && state.version !== STATE_VERSION) needsReset = true;
    if (state && (!Array.isArray(state.machines) || state.machines.length === 0)) needsReset = true;
    if (
      state &&
    ...
    if (state && !Array.isArray(state.rooms)) needsReset = true;
    if (state && !Array.isArray(state.bookings)) needsReset = true;

    if (needsReset) {
      state = buildDefaultState();
    } else {
      // Top-up missing arrays in case a prior schema write removed them.
      if (!Array.isArray(state.washes)) state.washes = [];
      if (!Array.isArray(state.notices)) state.notices = [];
      if (!Array.isArray(state.reports)) state.reports = [];
      if (!Array.isArray(state.machines) || state.machines.length === 0) state.machines = makeMachines();
      if (!Array.isArray(state.rooms) || state.rooms.length === 0) state.rooms = makeRooms();
      if (!Array.isArray(state.bookings)) state.bookings = [];
      if (!state.watchFree) state.watchFree = makeWatchFree(state.machines);
    }
    ...
    console.error('initState error: resetting demo data', err);
    hardReset();
  }
}
